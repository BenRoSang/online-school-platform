...
export function LessonPlayerPage() {
  ...
  return <div className="mx-auto grid w-full max-w-[1500px] flex-1 lg:grid-cols-[320px_1fr]">
    <aside className="border-b border-slate-200 bg-white lg:border-b-0 lg:border-r"><div className="sticky top-0 max-h-[45vh] overflow-y-auto p-5 lg:max-h-screen"><Link to={`/courses/${course.slug}`} className="text-sm font-semibold text-brand-700">← Course details</Link><h1 className="mt-4 text-xl font-bold text-slate-950">{course.title}</h1><nav className="mt-6 space-y-5" aria-label="Course curriculum">{course.sections.map((section) => <section key={section.id}><h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">{section.title}</h2><ol className="mt-2 space-y-1">{section.lessons.map((item) => <li key={item.id}>{item.accessible ? <Link to={`/courses/${course.slug}/learn/${item.id}`} className={`block rounded-lg px-3 py-2 text-sm ${item.id === lesson.id ? 'bg-brand-50 font-bold text-brand-700' : 'text-slate-700 hover:bg-slate-50'}`}>{item.title}{item.isPreview && <span className="ml-2 text-xs text-emerald-700">Preview</span>}</Link> : <span className="flex items-center justify-between rounded-lg px-3 py-2 text-sm text-slate-400"><span>{item.title}</span><span aria-label="Locked">🔒</span></span>}</li>)}</ol></section>)}</nav></div></aside>
    <main className="min-w-0 bg-slate-950 text-white"><div className="mx-auto max-w-5xl p-4 sm:p-8 lg:p-10"><div className="aspect-video overflow-hidden rounded-xl bg-black shadow-2xl"><iframe src={`https://www.youtube-nocookie.com/embed/${lesson.youtubeVideoId}`} title={lesson.title} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /></div><div className="mt-8"><div className="flex flex-wrap items-center gap-3">{lesson.isPreview && <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-bold text-emerald-300">Preview lesson</span>}</div><h2 className="mt-3 text-3xl font-bold tracking-tight">{lesson.title}</h2><p className="mt-4 max-w-3xl leading-7 text-slate-300">{lesson.description}</p><nav className="mt-10 flex justify-between border-t border-white/10 pt-6" aria-label="Lesson navigation">{previousLessonId ? <Link to={`/courses/${course.slug}/learn/${previousLessonId}`} className="rounded-lg border border-white/20 px-4 py-2 font-semibold hover:bg-white/10">← Previous</Link> : <span />}{nextLessonId && <Link to={`/courses/${course.slug}/learn/${nextLessonId}`} className="rounded-lg bg-brand-500 px-4 py-2 font-semibold hover:bg-brand-600">Next →</Link>}</nav></div></div></main>
  </div>
}
